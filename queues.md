...
The `handle` method is called when the job is processed by the queue. Note that we are able to type-hint dependencies on the `handle` method of the job. The Lumen [service container](/docs/{{version}}/container) automatically injects these dependencies.
